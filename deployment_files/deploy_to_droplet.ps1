...
Function Invoke-SshCommand {
    ...
    try {
        ...
        $originalException = $_
        $errorMsgToLog = "FATAL: Overall failure in Invoke-SshCommand for '$capturedActionDescription'. $($originalException.Exception.Message)"
        Write-Log -Message $errorMsgToLog -Level "ERROR" -LogFilePath $BuildLog
        "[$([DateTime]::UtcNow.ToString('o'))] CATCH_BLOCK Invoke-SshCommand for '$capturedActionDescription'. Error: $($originalException.Exception.ToString())" | Out-File -Append -FilePath $eventActionDebugLogPath -Encoding utf8
    ...
    "[$([DateTime]::UtcNow.ToString('o'))] End of Invoke-SshCommand for '$capturedActionDescription'. outputBuilder Length: $($outputBuilder.Length), errorBuilder Length: $($errorBuilder.Length)" | Out-File -Append -FilePath $eventActionDebugLogPath -Encoding utf8
